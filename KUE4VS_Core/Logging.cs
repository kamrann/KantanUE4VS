...
namespace KUE4VS
{
	public static class Logging
	{
		private static readonly string LogFolderPathRoot =
			Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "KantanDev");

		private const string LogFileNameBase = "KantanUE4VS-log";
		...
		private static void Initialize(int FileSuffix)
		{
			...
			catch (Exception ex)
			{
				...
				bLoggingReady = false;
				throw new ApplicationException("Failed to init logging in KantanUE4VS", ex);
			}
		}
